...
start_process (void *file_name_)
{
  ...
  // P3: init supplemental page table in this thread.
  spage_table_init (&thread_current ()->spage_table);
...
{
  struct child_process* cp = process_get_child (child_tid);
  if (!cp || (cp->status_reg & 1)) return -1;
  cp->status_reg |= 1;
  if (!(cp->status_reg & (1 << 3))) sema_down (&cp->sema_child_exit);
  int status = cp->ret;
  list_remove (&cp->elem);
  free (cp);
  return status;
...
process_exit (void)
{
  ...
  // P2: close all files of the current thread
  struct list_elem *next;
  ...
  // P2: remove all child processes of the current thread
  ...
  if (is_thread_alive (t->parent) && t->cp && t->executable)
    {
      t->cp->status_reg |= (1 << 3);
      sema_up(&t->cp->sema_child_exit);
    }

  // P3: unmap all memory mapped file of the process before exitting
  mmap_unmap (-1);

  // P3: free the supplemental page table of the process
  spage_table_free (&t->spage_table);
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      // P3: add the page to supplemental page table.
      if (!spage_table_put_file (file, ofs, upage, page_read_bytes, page_zero_bytes, writable, FILE))
	return false;
